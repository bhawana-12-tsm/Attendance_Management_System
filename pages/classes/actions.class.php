...
class Actions{
    ...
    public function delete_class(){
        ...
        if($delete){
            ...
            'msg' => "Class has failed to deleted due to unknown reason!" ];
     ...
     */
     ...
     public function save_student(){
        ...
        if(!empty($id)){
            $check = $this->conn->query("SELECT id FROM `students_tbl` 
            where `name` = '{$name}' and `class_id` = '{$class_id}' and `id` != '{$id}' ");
            $sql = "UPDATE `students_tbl` set `name` = '{$name}', `class_id` = '{$class_id}' 
            where `id` = '{$id}'";
        }else{
            
            $check = $this->conn->query("SELECT id FROM `students_tbl` where `name` = '{$name}' 
    ...
    public function delete_student(){
        ...
        if($delete){
            ...
            $_SESSION['flashdata'] = [ 'type' => 'danger', 
            'msg' => "Student has failed to deleted due to unknown reason!" ];
    ...
    public function list_student(){
        $sql = "SELECT `students_tbl`.*, `class_tbl`.`name` as `class` FROM `students_tbl` 
        inner join `class_tbl` on `students_tbl`.`class_id` = `class_tbl`.`id` order by `students_tbl`.`name` ASC";
        $qry = $this->conn->query($sql);
        return $qry->fetch_all(MYSQLI_ASSOC);
    }
    ...
    public function save_attendance(){
        ...
        foreach($student_id as $k => $sid){
            ...
            $check = $this->conn->query("SELECT id FROM `attendance_tbl` where `student_id` = '{$sid}' 
            ...
            if($check->num_rows > 0){
                ...
                try{
                    $update = $this->conn->query("UPDATE `attendance_tbl` set `status` = '{$stat}' 
                    where `id` = '{$att_id}'");
        ...
        {
            try{
                $sql =  $this->conn->query("INSERT INTO `attendance_tbl` ( `student_id`, `class_date`, `status` ) 
                VALUES {$sql_values}");
            }catch(Exception $e){
                if(!empty($errors)) $errors .= "<br>";
                $errors .= $e->getMessage();
